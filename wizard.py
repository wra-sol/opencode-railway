...
{hint}
    ...
    def _manage_get(self, path):
        ...
        # Wave 5.1: config read view
        if path == "/manage/config":
            return self._render_config_view()
        # Wave 5.2: backup download
        if path == "/manage/backup":
            return self._backup_download()
        return self._send(404, "not found", "text/plain")
    ...
    # ── Wave 5.1: /manage/config read view ──────────────────────────────────────

    def _render_config_view(self):
        """Show rendered opencode.json (with {env:VAR} placeholders) and masked .setup.env."""
        # Read opencode.json (with placeholders intact — no raw keys)
        oc_json_path = os.path.join(self.data_dir, "opencode.json")
        oc_json_text = "(not yet generated — run /setup or restart)"
        if os.path.exists(oc_json_path):
            try:
                ...
                try:
                    parsed = json.loads(oc_json_text)
                    oc_json_text = json.dumps(parsed, indent=2)
                except json.JSONDecodeError:
                    pass  # show raw
            except OSError:
                pass

        # Read .setup.env with API keys masked
        setup_env = load_existing(self.data_dir)
        masked_lines = []
        secret_key_patterns = ("API_KEY", "TOKEN", "PASSWORD", "SECRET", "PAT")
        for key, val in sorted(setup_env.items()):
            if any(p in key.upper() for p in secret_key_patterns) and val:
                masked = val[:4] + "…" + val[-4:] if len(val) > 8 else "****"
                masked_lines.append(f"{key}={masked}")
            else:
                masked_lines.append(f"{key}={val}")
        masked_env = "\n".join(masked_lines) if masked_lines else "(empty)"

        page = f"""<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<style>{CSS}</style></head><body><div class="wrap">
<div class="brand"><div class="brand-mark"></div><div class="brand-name">opencode</div></div>
<p class="sub">Railway manager &middot; config</p>
<h3 style="margin-top:20px;color:var(--accent-hi)">opencode.json</h3>
<p class="sub" style="margin-bottom:8px">Generated by <code>generate_config.py</code> on every boot. API keys appear as <code>{{env:VAR}}</code> placeholders — not raw values.</p>
<pre class="logs">{html.escape(oc_json_text)}</pre>
<h3 style="margin-top:24px;color:var(--accent-hi)">.setup.env (masked)</h3>
<p class="sub" style="margin-bottom:8px">Secrets are masked. Use <a href="/manage/backup">backup</a> to download the raw file (requires re-auth).</p>
<pre class="logs">{html.escape(masked_env)}</pre>
<p class="sub" style="margin-top:18px"><a href="/manage">&larr; Back to dashboard</a></p>
</div></body></html>"""
        self._send(200, page)

    # ── Wave 5.2: backup/restore ────────────────────────────────────────────────
    ...
    def _backup_download(self):
        """Stream a tarball of config files for backup."""
        import io
        import tarfile
        ...
        self.send_header("Content-Disposition", "attachment; filename=opencode-backup.tar.gz")
        self.send_header("Cache-Control", "no-store")
        for k, v in _security_headers(self).items():
            self.send_header(k, v)
        self.send_header("Connection", "close")
        ...
        try:
            ...
            pass
        self.manager.audit.record(self._current_user() or "unknown", _client_ip(self), "backup_download")

    def _restore_upload(self, raw):
        """Accept a tarball upload and restore config files."""
        import io
        import tarfile
        ...
        try:
            ...
            with tarfile.open(fileobj=buf, mode="r:gz") as tar:
                ...
                # Validate: at least .setup.env should be present
                if ".setup.env" not in members:
                    return self._json(400, {"ok": False, "error": "Backup tarball must contain .setup.env"})
                # Extract only known files (don't allow arbitrary path traversal)
                for name in self._BACKUP_FILES:
                    if name in members:
            ...
            return self._json(400, {"ok": False, "error": f"Invalid backup: {e}"})

        self.manager.audit.record(user, ip, "restore_upload")
        self.manager.log_event("info", "backup restored — applying settings + restarting")
    ...
    def _manage_post(self, path, f, f_multi, raw):
        ...
        if path == "/manage/users/remove":
            ...
            return self._redirect("/manage")
        # Wave 5.2: restore upload
        if path == "/manage/restore":
            return self._restore_upload(raw)
